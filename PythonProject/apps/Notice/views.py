...
class FrontAddView(BaseView):  # 前台站内通知添加
...
class FrontListView(BaseView):  # 前台站内通知查询列表
    ...
    def handle(self, request):
        ...
        if addTime != '':
            notices = notices.filter(addTime__contains=addTime)
...
class FrontShowView(View):  # 前台显示站内通知详情页
    def get(self, request, noticeId):
        # 查询需要显示的站内通知对象
...
class ListAllView(View): # 前台查询所有站内通知
    def get(self,request):
        notices = Notice.objects.all()
        noticeList = []
...
class UpdateView(BaseView):  # Ajax方式站内通知更新
    def get(self, request, noticeId):
        # GET方式请求查询站内通知对象并返回站内通知json格式
        notice = Notice.objects.get(noticeId=noticeId)
        return JsonResponse(notice.getJsonObj())

    def post(self, request, noticeId):
        # POST方式提交站内通知修改信息更新到数据库
        notice = Notice.objects.get(noticeId=noticeId)
        notice.title = request.POST.get('notice.title')
        notice.content = request.POST.get('notice.content')
        notice.addTime = request.POST.get('notice.addTime')
...
class AddView(BaseView):  # 后台站内通知添加
    def get(self,request):
...
class BackModifyView(BaseView):  # 后台更新站内通知
    def get(self, request, noticeId):
        context = {'noticeId': noticeId}
        return render(request, 'Notice/notice_modify.html', context)


class ListView(BaseView):  # 后台站内通知列表
    def get(self, request):
        # 使用模板
...
class DeletesView(BaseView):  # 删除站内通知信息
    def get(self, request):
        return self.handle(request)
...
class OutToExcelView(BaseView):  # 导出站内通知信息到excel并下载
    def get(self, request):
        # 收集查询参数
        title = self.getStrParam(request, 'title')
        addTime = self.getStrParam(request, 'addTime')
