...
class FrontAddView(BaseView):  # 前台用户添加
    ...
    def post(self, request):
        ...
        userInfo.areaObj = Area.objects.get(areaId=request.POST.get('userInfo.areaObj.areaId'))
...
class FrontListView(BaseView):  # 前台用户查询列表
    ...
    def handle(self, request):
        ...
        areaObj_areaId = self.getIntParam(request, 'areaObj.areaId')
        ...
        if areaObj_areaId != '0':
            userInfos = userInfos.filter(areaObj=areaObj_areaId)
        if name != '':
        ...
        context = {
            ...
            'user_name': user_name,
            'areaObj_areaId': int(areaObj_areaId),
            'name': name,
            'birthday': birthday,
            'telephone': telephone,
