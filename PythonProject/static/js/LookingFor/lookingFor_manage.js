				...
				if ($("#lookingForEditForm").form("validate")) {
					...
					if(!$("#lookingForEditForm").form("validate")) {
						...
						$("#lookingForEditForm").form({
						    url:"/LookingFor/update/" + $("#lookingFor_lookingForId_edit").val(),
						    onSubmit: function(){
								if($("#lookingForEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	lookingFor_manage_tool = {
		...
		exportExcel: function() {
			$("#lookingForQueryForm").form({
			    url:"/LookingFor/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/LookingFor/update/" + rows[0].lookingForId,
					type : "get",
					data : {
