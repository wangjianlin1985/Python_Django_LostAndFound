				...
				if ($("#lostFoundEditForm").form("validate")) {
					...
					if(!$("#lostFoundEditForm").form("validate")) {
						...
						$("#lostFoundEditForm").form({
						    url:"/LostFound/update/" + $("#lostFound_lostFoundId_edit").val(),
						    onSubmit: function(){
								if($("#lostFoundEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
	...
	lostFound_manage_tool = {
		...
		exportExcel: function() {
			$("#lostFoundQueryForm").form({
			    url:"/LostFound/OutToExcel?csrfmiddlewaretoken" + $('input[name="csrfmiddlewaretoken"]').val(),
			});
			//提交表单
		...
		edit : function () {
			...
			if (rows.length > 1) {
				...
				$.ajax({
					url : "/LostFound/update/" + rows[0].lostFoundId,
					type : "get",
					data : {
					...
					success : function (lostFound, response, status) {
						...
						if (lostFound) { 
							...
							$("#lostFound_userObj_user_name_edit").combobox({
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#lostFound_userObj_user_name_edit").combobox("select", lostFound.userObjPri);
